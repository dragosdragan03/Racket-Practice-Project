...
(define (get-ch-words words ch)
  (filter (lambda (first-ch) (char=? (car first-ch) ch)) words))
...
(define (ast-func suffixes)
  (cons (list (caar suffixes)) (map (lambda (word) (drop word 1)) suffixes)))
...
(define (cst-func suffixes)
  (let* ([longest-prefix (longest-common-prefix-of-list suffixes)]
         [list-suffixes (map (lambda (word) (drop word (length longest-prefix))) suffixes)])
...
(define (branch labeling-func list-suffixes alphabet)
  (cond
    [(null? list-suffixes) '()]
    [else (cons (first (labeling-func list-suffixes)) ; construiesc lista dintre prefix si restul de sufixe
                        (suffixes->st labeling-func ; apelez functia suffixes->st cu sufixele ce au ramas
                                      (cdr (labeling-func list-suffixes))
                                      alphabet))]))
...
(define (suffixes->st labeling-func suffixes alphabet)
  (filter (lambda (lst) (not (null? lst)))
          (map (lambda (letter) 
                 ...
                 (get-ch-words (filter (lambda (lst) (not (null? lst))) suffixes) letter) ; extrag doar sufixele care incep cu letter
                 alphabet))
       alphabet)))
...
(define (text->st labeling-func) ; fac o functie de tip curry
  (lambda (text)
    (let* ((lista (get-suffixes (append text (list #\$)))) ; creez lista cu toate sufixele cuvantului
           (alphabet (sort (remove-duplicates (apply append lista)) char<?))) ; creez lista de litere
      (suffixes->st labeling-func lista alphabet))))
