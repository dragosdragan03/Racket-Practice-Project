...
(define (get-ch-branch st ch)
  (if (st-empty? st)
      #f ; daca este gol atunci nu s a gasit litera 
      (if (equal? (car (get-branch-label (first-branch st))) ch)
          (first-branch st) ; trebuie sa fac ramura care imi face arborele
          (get-ch-branch (other-branches st) ch)))) ; dau ca parametru restul listei de noduri
