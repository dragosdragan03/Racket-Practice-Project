...
(define (longest-common-prefix w1 w2)
  (let* ([prefix-words (prefix w1 w2 '())])
    (append (list prefix-words)
            (list (drop w1 (length prefix-words)))
            (list (drop w2 (length prefix-words))))))
...
(define (longest-common-prefix-of-list words)
  (if (or (= (length words) 1) (= 1 (length (car words))))
      (car words)
...
(define (st-has-pattern? st pattern)
  ...
  (cond
    ...
    (else (st-has-pattern? (car (cddr result)) (car (cdr result)))))))
